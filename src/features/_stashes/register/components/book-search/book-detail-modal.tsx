...
type BookDetailModalProps = {
  visible: boolean;
  book: BookSearchItemType | null;
  onClose: () => void;
  onRegister: (book: BookSearchItemType, targetURL: string) => Promise<void>;
};
...
export const BookDetailModal = ({
  ...
  useEffect(() => {
    if (book) {
      setTargetURL(`https://books.google.com/books?id=${book.id}`);
    }
  }, [book]);
        ...
        <ScrollView style={modalStyles.content}>
          <View style={modalStyles.bookDetailContainer}>
            ...
            <View style={modalStyles.bookInfo}>
              <Text style={modalStyles.bookTitle}>{volumeInfo.title}</Text>
          ...
          {volumeInfo.description && (
            <View style={modalStyles.descriptionContainer}>
              ...
              <Text style={modalStyles.descriptionText}>
                {sliceTxt(volumeInfo.description.replace(/<[^>]*>/g, ""), 120)}
              </Text>
            </View>
          )}
